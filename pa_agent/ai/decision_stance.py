...
def build_decision_stance_guidance(stance: str | None) -> str:
    ...
    common_rules = (
        "通用约束（各档都必须遵守）：\n"
        "- 仍必须完整输出 decision_trace，按 §9–§10、§14 走适用节点；§11 由程序生成。\n"
        ...
        "- **唯一入场意图**：pullback=等待结构位、breakout=等待极值突破、"
        "immediate=已确认立即入场、none=无有效方案；不得默认选择或静默换型。\n"
        "- 突破锚点缺失、触发已经越过、实时价格关系错误时，当前方案直接拒绝；"
        "禁止自动转为限价或市价。\n"
        "- **direction=neutral 约束**：仅允许顺 §2.4 的单一方向意图；"
        "§2.4=否时使用 none，禁止上下边界双向刮头皮。\n"
        "- **方案连续性**：若上一轮可执行方案**未失效**，默认 wait；"
        "同结构位 N 根内禁止反手（详见阶段二「方案连续性」块）。\n"
        "- pending 意图先对照 K1.close，最终由程序使用最新形成中 K 线报价核验价格关系。\n"
    ...
    if normalized == "conservative":
        ...
        profile = (
            ...
            "- 周期与方向负责定义 setup，触发状态负责定义 entry_intent；不存在默认订单类型。\n"
            "- §9 入场：除典型信号外，若结构与阶段一 direction/cycle_position 一致，"
            "允许「次优但可执行」的二类 setup（须在 reason 中写明为何仍值得做）。\n"
            "- 宽通道/区间边界可形成 pullback，也可在确认后形成 breakout/immediate；"
            "必须按本轮实际触发条件选择，不得只凭周期命名。\n"
            ...
            "- 顺势通道、尖峰延续、区间边界反弹都必须说明为何是等待回撤、等待突破或立即执行。\n"
        )
        ...
        profile = (
            ...
            "- 任一意图不可行时当前方案直接失败；禁止为了产出交易改成另一种订单。\n"
